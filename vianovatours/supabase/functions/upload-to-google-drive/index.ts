...
const getConnectorAccessToken = async (provider: string) => {
  const normalizedProvider = provider.toLowerCase();
  const providerKey = toSnakeUpper(normalizedProvider);
  const candidates = [`${providerKey}_ACCESS_TOKEN`, `${providerKey}_TOKEN`];
  ...
  for (const key of candidates) {
    ...
    if (value) {
      if (normalizedProvider === "googledrive" && isTokenLikelyExpired(value)) {
        break;
      }
      return value;
    }
